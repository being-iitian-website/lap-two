 ...
 */
export const saveVisionBoard = async (
  ...
  try {
    ...
    const { columns, rows, gap, items } = req.body as SaveVisionBoardBody;
 ...
 */
export const getVisionBoard = async (
  req: AuthenticatedRequest,
  res: Response
): Promise<Response | void> => {
  try {
    ...
    const board = await (prisma as any).visionBoard.findFirst({
      where: { userId },
      include: {
        items: {
          orderBy: [
